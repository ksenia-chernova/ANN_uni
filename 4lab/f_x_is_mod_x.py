...
x_range_dense = np.linspace(-20, 20, 1000).reshape(-1, 1)
y_true_dense = np.abs(x_range_dense)
predictions_dense = model.predict(x_range_dense)

plt.plot(x_range_dense, y_true_dense, 'b-', label='Истинная функция: f(x) = |x|', linewidth=2, alpha=0.7)
plt.scatter(x_range_dense, predictions_dense, color='red', s=1, label='Предсказания нейронной сети', alpha=0.6)
plt.xlabel('X')
plt.ylabel('Y')
...
plt.subplot(1, 3, 2)
control_sorted_indices = np.argsort(x_control[:, 0])
x_control_sorted = x_control[control_sorted_indices]
y_control_sorted = y_control[control_sorted_indices]
control_predictions_sorted = control_predictions[control_sorted_indices]

control_errors = y_control_sorted - control_predictions_sorted
plt.plot(x_control_sorted, control_errors, 'g-', linewidth=1, alpha=0.7)
plt.scatter(x_control_sorted[::40], control_errors[::40], color='green', s=15, alpha=0.6)
plt.axhline(y=0, color='black', linestyle='--', linewidth=1)
plt.fill_between(x_control_sorted[:, 0], control_errors[:, 0], 0, alpha=0.2, color='green')
plt.xlabel('X')
plt.ylabel('Ошибка')
